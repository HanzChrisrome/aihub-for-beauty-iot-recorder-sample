...
// InitPortAudio ensures PortAudio is initialized only once globally
...
func (af *AIFFAudioFormat) Record() {
    ...
    // Ensure PortAudio is initialized (safe for concurrent calls)
    ...
    // Use specific device if index is set, otherwise use default
    if af.DeviceIndex >= 0 {
        // Get all devices
        ...
        if inputDevice.MaxInputChannels == 0 {
            panic(fmt.Sprintf("device %d (%s) has no input channels", af.DeviceIndex, inputDevice.Name))
        }
        ...
        if err != nil {
            panic(fmt.Sprintf("Failed to open device %d (%s): %v\nDevice may already be in use", 
                af.DeviceIndex, inputDevice.Name, err))
        }
    } else {
        // Use default input device (backward compatibility)
